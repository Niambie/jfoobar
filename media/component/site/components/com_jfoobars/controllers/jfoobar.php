...
class JfoobarsControllerJfoobar extends JControllerForm
{
	 ...
	 *
	 * @return	boolean	True if the jfoobar can be added, false if not.
	 * @since	1.6
	 */
	public function add()
	{
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		$user		= JFactory::getUser();
		$userId		= $user->get('id');
		$asset		= 'com_jfoobars.jfoobar.'.$recordId;
...
//		}
	...
	protected function postSaveHook(JModel &$model, $validData)
	{
		...
		if ($task == 'save') {
			$this->setRedirect(JRoute::_('index.php?option=com_jfoobars&view=category&id='.$validData['catid'], false));
		}
	}
	...
	public function save($key = null, $urlVar = 'a_id')
	{
		...
		require_once JPATH_ADMINISTRATOR.'/components/com_jfoobars/helpers/jfoobars.php';
	...
	function vote()
	{
		...
		if ( $user_rating > -1 ) {
			...
			if ($model->storeVote($id, $user_rating)) {
				$this->setRedirect($url, JText::_('COM_JFOOBARS_JFOOBAR_VOTE_SUCCESS'));
			} else {
				$this->setRedirect($url, JText::_('COM_JFOOBARS_JFOOBAR_VOTE_FAILURE'));
			}
		}
	}
}